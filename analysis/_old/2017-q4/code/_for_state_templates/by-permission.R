...
## 1. Participant counts (count of records in history table)
part <- list(
    "tot" = est_part(priv)
)
for (i in c("res", "sex", "agecat")) {
    part[[i]] <- est_part(priv, i) %>% 
        scaleup_part(part[["tot"]])
}


## 2. Resident counts (mostly intermediate results for rate estimation)
priv_resident <- priv %>%
    filter(res == "Resident")
...
for (i in c("sex", "agecat", "county")) {
    part_resident[[i]] <- est_part(priv_resident, i) %>% 
        scaleup_part(part_resident[["tot"]], flag_na = 10)
} 
# part[["county"]] <- part_resident[["county"]]


## 3. Participation rate (pop / part)
rate <- list()
for (i in names(part_resident)) {
    pop <- est_pop(pop_county, i)
...
## 4. New recruits (filter part by R3)
priv_recruit <- priv %>%
...
for (i in c("res", "sex", "agecat")) {
    part_recruit[[i]] <- est_part(priv_recruit, i) %>% 
        scaleup_part(part_recruit[["tot"]])
} 
# county-specific not needed for nat/reg
# part_recruit[["county"]] <- est_part(priv_recruit, "county", flag_change = 40) %>%
#     scaleup_part(part_recruit[["res"]])
...
for (i in c("res", "sex", "agecat")) {
    churn[[i]] <- est_churn(priv, i)
}
# churn[["county"]] <- est_churn(priv, "county", flag_change = 40) # not needed for nat/reg
